...
use slog::{debug, info, warn, o, Logger};
...
/// The "initializer" for the service/the actual middleware (called once per worker).
...
/// The "handler" for the response, i.e. "action" to call once the other services are done, and the
/// response is ready.
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
        ...
        if let Some(error) = res.response().error() {
            if res.response().head().status != StatusCode::INTERNAL_SERVER_ERROR {
                warn!(this.logger, "Error in response: {:?}", error);
            }
        }
...
impl<B> PinnedDrop for StreamLog<B> {
    fn drop(self: Pin<&mut Self>) {
        if !self.is_exclude {
            ...
            debug!(self.logger, "-"; o!("bytes_sent" => self.size, "response_time" => response_time));
        }
    }
}
